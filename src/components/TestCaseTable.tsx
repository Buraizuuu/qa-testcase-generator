      ...
      <div className="flex flex-wrap gap-2">
        ...
        <div className="relative">
          <select
            ...
            className="appearance-none bg-slate-800 border border-slate-700 rounded-lg pl-3 pr-8 py-2 text-sm text-slate-300 focus:outline-none focus:ring-2 focus:ring-blue-500 min-h-[44px] cursor-pointer"
            aria-label="Filter by category"
          >
            <option value="All">All Categories</option>
            {ALL_CATEGORIES.map((c) => <option key={c} value={c}>{c}</option>)}
          </select>
          <svg className="pointer-events-none absolute right-2.5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" aria-hidden="true">
        ...
        <table className="w-full text-sm" role="grid" aria-label="Test cases">
          <thead>
            <tr className="border-b border-slate-800 bg-slate-900/50">
              ...
              <th className="text-left px-3 py-3 text-xs font-semibold text-slate-400 uppercase tracking-wider whitespace-nowrap">Automate?</th>
